...
# role's purpose list, (3) idle-monitor is in manager's list and the legacy
...
# Case 3: idle-monitor IS in manager's purpose list.
HAS_IDLE=$(jq -r '.manager | (index("idle-monitor") != null)' "$MAP")
assert_eq "case-3-idle-monitor-in-manager" "true" "$HAS_IDLE"

# Case 4: manager-monitor is NOT in manager's purpose list (regression guard).
HAS_LEGACY=$(jq -r '.manager | (index("manager-monitor") != null)' "$MAP")
assert_eq "case-4-no-legacy-manager-monitor" "false" "$HAS_LEGACY"
...
# Case 6: manager's list contains exactly bus-tail + github-bridge + idle-monitor
