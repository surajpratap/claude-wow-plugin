...
sleep 0.3
ERR_C=$(WOW_ROOT="$PC" bash "$VERIFY_HELPER" 2>&1 >/dev/null)
RC_C=$?
...
BG_D=$!
sleep 1
WOW_ROOT="$PD" bash "$VERIFY_HELPER" 2>/dev/null
RC_D=$?
...
WOW_ROOT="$PE" bash "$RECORD_HELPER" bus-tail "fake-rearm-task-id" 2>/dev/null
