...
# -----------------------------------------------------------------------------
# Case 7 (Story 069 amendment-3): cross-check that a trigger-type emit
# (story-created) writes 2 lines via this suite's fixture path. The dedicated
# auto-inject suite covers behavior in depth; this guard catches a regression
# where the auto-inject branch silently degrades.
# -----------------------------------------------------------------------------
P7=$(mk_project)
...
assert_eq "case-7-trigger-emits-2-lines" "2" "$LINES7"
