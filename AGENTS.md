...
- **PreToolUse `rm`+glob block for non-M (Layer E)** — `scripts/hooks/wow-block-rm-glob-non-manager.sh` matches `\brm\b` + any of `*`/`?`/`[` in a Bash command; resolves the role via `whats-my-role.sh`; M is exempt (user-facing, the human approves CC's permission prompt directly). Non-M roles get an immediate `{decision: "block"}` JSON with remediation pointers (`find -delete`, single-file `rm -f`, or M-nudge bypass for legitimate cases) instead of silently stalling on a CC permission prompt they cannot answer. Regex biases to false positives by design — see `commands/_agent-protocol.md` "Hooks" section for the full remediation pattern.
