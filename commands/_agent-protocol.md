...
| Pair Programmer  | `/pair-programmer`  | Inline review blocks in plans/stories; `.review.txt` finding entries; bug triage blocks; bus messages; `gh pr comment <N> --repo <owner>/<repo> --body ...` replies on watched PRs | Production code, plans, stories, test-stories, bug files directly            |
...
${ROOT}/
  implementations/
    ...
    config.json                 # consolidated mode state, wow-config.sh-owned
                                # {"schema":1,"mode":"default"|"ahod","ahod":{...}}
...
The `bus_emit` MCP **input schema** and the **on-disk serialized output** diverge for one field today, and the divergence is intentional. Anyone writing `jq` against `${ROOT}/implementations/.message-bus.jsonl` MUST match the on-disk shape — NOT the MCP input shape — or filters silently become always-true / always-false.
...
| `wake` | Truly-idle nudge from `manager-monitor.py`. Fires when a non-M peer's latest activity row is terminal (`stop` / `stop_failure`) AND older than `PER_ROLE_IDLE_SECONDS` (default 600s). Idempotency: don't re-wake the same agent within `PER_ROLE_REWAKE_SECONDS` (default 1800s) — tracked in `implementations/.wow-process/manager-monitor-last-wake.json`. Recipient re-scans bus for missed events + runs the wake-loop self-check; resumes work or emits `status` confirming idle. Closes 099's truly-idle limitation mechanically. | manager-monitor (self-emit) → recipient agent's exact ID |
| `merge-authority-grant` | S relays a human merge-authority grant as a CANDIDATE. Payload: parser output `{candidate:true, scope:<this-sprint\|per-item\|final-integration\|unscoped>, raw}` from `scripts/merge-authority-parse.sh` (detect-candidate-only, fail-CLOSED). NEVER an active grant — M must get explicit human confirm. | `slacker-*` → `manager-*` |
| `merge-authority-ack` | M's structured, auditable ack of a grant candidate. Payload: `{state:<pending\|active\|revoked>, scope, sprint}`. On a candidate M sets `merge_authority.state=pending` in the manifest + ALWAYS asks the human (via S) to confirm; only on explicit confirm → `active`; M exercises merge authority ONLY when `active`. | `manager-*` → `slacker-*` |
...
| `learnings-consolidated`| Peer reports running `consolidate-memory.sh` (slash command, startup phase, or retro `nudge` with `repair: consolidate-memory`). Payload: `{role, path, entries_added, entries_skipped, triage_count}`. Always emitted on script invocation, including no-op / triage-only runs. Distinct from `learnings-updated` (different payload shape, different trigger surface). | any peer → `manager-*`                   |
...
| `ahod-kickoff`          | M starts AHOD mode — every agent owns one item end-to-end in parallel. Payload: `{pool: [<story refs>], assignments: {<role>: <story ref>}, doctrine: "commands/_ahod-doctrine.md"}`. Auto-injects fire alongside it (`read-token-discipline`, `read-learnings`, `read-ahod-doctrine`). | M → `*`                                  |
| `ahod-ack`              | Peer acknowledges an `ahod-kickoff` after re-reading the AHOD doctrine + its assignment. Payload: `{role, ts}` (peers may add a `note`). | any peer → `manager-*`                   |
| `ahod-stand-down`       | M ends AHOD mode after the human's `/ahod-off` wind-down. Payload: `{reason, wind_down}` — the per-item finish-to-PR / park summary. Normal relay resumes on receipt. | M → `*`                                  |
| `read-ahod-doctrine`    | Refresh signal — peers re-read `commands/_ahod-doctrine.md` from disk. Auto-injected by the MCP server (`mcp/claude-wow-server/server.py`) on every `ahod-kickoff`, and on `story-created` / `compaction-occurred` while `implementations/config.json` has `mode: "ahod"` (recipient mirrors the trigger's `to`). Payload: `{path: "commands/_ahod-doctrine.md", reason: "auto-injected after <type>"}`. | <auto-injected by MCP server> → `*` or mirrors the trigger's `to` |
...
- On `payload.directive == "pause"`: **HALT all work immediately and ignore other nudges** until a matching resume arrives. Do not start, continue, or commit work while paused.
  - **`kill_subagents` (optional pause field).** When the `pause` payload carries `kill_subagents: true` (an urgent stop — e.g. M's reaction to a `manager_monitor` `usage-limit`), then BEFORE halting, `TaskStop` every subagent + ephemeral work-Monitor you spawned **this session** — the token-burning work — and only then halt. To do this, each role keeps an **in-session list of the task IDs it spawned** (via the `Agent` tool or an ephemeral `Monitor`) and `TaskStop`s each one. **EXEMPT — never kill — your own bus-tail Monitor:** it is your *resume lifeline*. `resume` is broadcast on the bus; a peer that killed its bus-tail would be permanently deaf to `resume` (it could never hear the lift — human-only recovery), and bus-tail costs ~0 tokens while idle. So bus-tail stays listening through the pause; you do NOT re-arm it on resume because it was never killed. `kill_subagents` absent or false → today's behavior (halt, leave subagents running). The field is backward-compatible.
- On `payload.directive == "resume"`: stop halting and continue normal work. (Your bus-tail — never killed — delivered this resume.)
...
- M is **the producer** of the urgent `pause` (`kill_subagents: true`) + the lifting `resume`, in reaction to the `manager_monitor` usage concern's piped `usage-limit` / `usage-reset` events (see `manager.md`). When M kills subagents on its own pause path, it **never kills its own `manager-monitor` or `bus-tail`** — it needs `manager-monitor` alive to detect the reset and `bus-tail` to stay on the bus.
- On `payload.directive == "escalate"` (a message addressed `to: manager-*`): M **escalates to the human** (`AskUserQuestion` / Slack) — surfacing the payload's `window` / `used_percentage` / `resets_at` — and the human decides whether to pause. M does not halt on it. (The usage signal now reaches M via the `manager_monitor` pipe — `usage-limit` / `usage-escalate` — rather than a bus `escalate`; a legacy bus `escalate` is still handled here.)
...
Each agent's hook router (`plugin/scripts/hooks/log-activity.sh`) writes one JSONL row to `implementations/.activity.jsonl` per registered hook event: `PreToolUse`, `UserPromptSubmit`, `Stop`, `StopFailure`, `SessionStart`, `SessionEnd`. Schema: `{ts, claude_pid, role, type, tool?, text?}`. `type` is one of `tool`, `bg-spawn`, `prompt_in`, `stop`, `stop_failure`, `session_start`, `session_end` — `bg-spawn` is logged on `PreToolUse` when the tool is a `Bash` call with `run_in_background: true` (a finite background command the peer `stop`s to await); every other tool call, including the `Monitor` tool, is `tool`.

A long-running manager-monitor at `plugin/scripts/wow-process/manager-monitor.sh` (started by M as a Monitor-tool task alongside bus-tail and the GitHub bridge) checks every 60s: for each live wow-process PID in the required set (`manager, senior-developer, pair-programmer, tester`), is its most recent activity row's `type` in `{stop, stop_failure}`? A peer at `stop`/`stop_failure` counts as idle **unless** its current stop-episode contains a `bg-spawn` — the run of rows since its previous `stop`/`stop_failure`/`session_start` — meaning it `stop`'d while awaiting backgrounded work; such a peer is `busy`. If every required peer is idle and `implementations/.nothing_to_do` is absent → print one JSONL `all-idle-nudge` line to stdout; CC forwards to M as a Monitor-task notification (not a bus message — M-private signal stays out of `.message-bus.jsonl`).

`.nothing_to_do` is a sticky do-not-disturb marker: **written** by the `declare_idle` MCP tool (M-only) and **cleared** either by the `resume_work` MCP tool (M-only) **or mechanically by the `wow-clear-idle-marker.sh` UserPromptSubmit hook on any user prompt (role-agnostic)** — a prompt deterministically means the team has work. The hook clears only this idle marker; it never reads or writes AFK state (`afk_active`). Idle (team-has-no-work) and AFK (human-away, M-judged) are orthogonal. The conversation surface (Claude's response text after a tool call) remains the user-facing signal that no-work mode changed state.

**`i_am_truly_idle` + gated `declare_idle`.** A peer (sd/pp/t) calls the `i_am_truly_idle` MCP tool when it has genuinely nothing to do; it records `{"<role>": {"idle": true, "ts": "<iso>", "pid": <pid>}}` in `implementations/.truly-idle.json` (idempotent per role; pid sourced from the role's most-recent `.activity.jsonl` `claude_pid`, so it matches what the gate checks). `declare_idle` is **gated**: it writes `.nothing_to_do` only if **sd + pp + t** each have an entry that is `idle:true`, pid-alive (`os.kill(pid,0)`), AND has no WORK-shaped `.activity.jsonl` row (excludes `stop`/`stop_failure`/`session_end`) since that entry's `ts` — otherwise it refuses and names the offender(s). A working agent auto-invalidates its own bit via that activity cross-check (no explicit unset). `resume_work` clears BOTH `.nothing_to_do` AND `.truly-idle.json` (the team re-affirms idle from scratch). Owner of the schema + gate = the MCP server (`server.py`); consumers = M via declare-or-nudge (`manager.md`) + the peers via the idle/`wake` handler.
...
**`integration_branch`:** optional top-level string naming the per-sprint integration branch (`sprint/<sprint-id>`). Set by M's sprint-kickoff when the sprint uses the integration-branch strategy (per-item PRs merge into the integration branch; integration→main PR consolidates at sprint-finalize). Consumed by the MCP server's `code-review` auto-injector to suppress per-item code-review when an active sprint's `pr_base` matches `integration_branch`. `sprint-manifest-validate.sh` shape-checks that the field, when present, is a non-empty string with the `sprint/` prefix; manifests without it are unaffected (back-compatible).
...
AHOD ("All Hands On Deck") is a human-activated team mode in which every agent — M included — owns one work item end-to-end in parallel; the serial relay is suspended and each owner self-drives story → plan → implement → gate → self-review → PR in its own worktree. The complete rulebook every agent follows is `commands/_ahod-doctrine.md`; the MCP server re-delivers it mechanically (`read-ahod-doctrine` auto-inject) on `ahod-kickoff` and — while the mode is active — on `story-created` and `compaction-occurred`.

- **State:** `implementations/config.json` (`{"schema": 1, "mode": "default" | "ahod", "ahod": {activated_ts, assignments}}`), read/written ONLY via `scripts/wow-config.sh` (atomic, worktree-invariant root). Missing file = `default`. M is the sole writer; assignments are keyed by role.
- **Entry/exit:** the `/ahod` and `/ahod-off` meta commands in M's terminal (`commands/_meta/ahod.md`, `commands/_meta/ahod-off.md`). The mode persists across sessions until revoked. Startup surfaces it: the env phase prints `env: mode=ahod`.
- **Dispatch:** `story-created` goes to the exact owner agent ID with payload `ahod: true`; the `pr-created` → `code-review-request` auto-inject routes to the PR author instead of `pair-programmer-*`.
- **Suspended vs in force:** the doctrine file's "Suspended in AHOD" and "Stays in force" sections are the authoritative lists. All hooks, liveness/idle machinery, token discipline, and the human-routing hard rule remain untouched.
- **Exclusivity:** AHOD and sprint mode never run together — `/ahod` refuses while a sprint manifest is `status: "active"`, and M refuses a sprint trigger while `mode` is `ahod`.
