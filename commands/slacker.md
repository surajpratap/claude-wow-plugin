...
- `wake` (from `manager-monitor-*`, to: your exact ID) → manager-monitor detected your role's latest activity row is terminal and older than `PER_ROLE_IDLE_SECONDS`. Re-scan bus for missed events; run the wake-loop self-check above; resume work or emit `status` confirming idle. Closes 099's truly-idle limitation.
