...
- `read-learnings` (to: `slacker-*`, your ID, or `*`) → re-read `implementations/learnings/slacker.md` from disk. Auto-injected by the MCP server on `story-created` / `sprint-kickoff` / `compaction-occurred` (Story 124). The `<role>` literal in `payload.path` is a template — substitute `slacker`.
