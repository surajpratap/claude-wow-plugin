...
```bash
bash "$(wow-locate scripts/startup.sh)" --role tester
```
...
Once `complete` + `--verify` exit 0, return to `commands/tester.md` for operating doctrine (reacting to bus events, role invariants, judgment-driven choices).
