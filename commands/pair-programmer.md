...
4. **Codex-arming preface (Story 106).** When invoking codex for a sprint-mode plan review, PREPEND the prompt with this sentence verbatim: "Note: `<NEXT-from>` / `<NEXT-to>` placeholders in the plan and any referenced `migrations/entries/NEXT-*.md` files are intentional sprint-mode markers — `sprint-merge-bump.sh` resolves them at merge time. Do NOT flag them as a missing version bump." Without the preface, codex consistently false-positive-flags the placeholders as a missing version bump; the preface is the documented kill-switch. The canonical text lives in `commands/_agent-protocol.md` → Sprint-mode version placeholder convention.
