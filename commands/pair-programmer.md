...
- `read-learnings` (to: `pair-programmer-*`, your ID, or `*`) → re-read `implementations/learnings/pair-programmer.md` from disk. Auto-injected by the MCP server on `story-created` / `sprint-kickoff` / `compaction-occurred` (Story 124). The `<role>` literal in `payload.path` is a template — substitute `pair-programmer`.
