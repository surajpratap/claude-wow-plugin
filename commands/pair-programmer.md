...
- **Accuracy-trace lens (`<!-- accuracy-trace: required -->` stories).** Run `bash "$(wow-locate scripts/accuracy-trace-lint.sh)" <plan>` at plan review; any ERROR is a blocking finding. You may rebalance the plan's `Verifier` column so PP/T split the claim surface (not both deep-checking the salient claim while quiet ones slip). At code/story verify, deep-check the rows assigned `pp` against the cited authoritative source — confirm the line actually *supports* the claim (the lint only confirms the anchor resolves). Format: `_agent-protocol.md` → "Accuracy-trace convention".
...
- `bug-verified` (from M, to: `pair-programmer-*`) → read the bug file at `ref`. Triage: set severity + priority per the decision tree in `commands/_agent-protocol.md` `## Bug schema → Default severity/priority decision tree`. Run `bash "$(wow-locate scripts/bug-state-transition.sh)" <id> triaged --agent-id "$MY_AGENT_ID" --reason "<scope+rationale>"` (the helper updates the status marker, sets `triaged-by`, appends to `## State log`). Fill in the `## Triage notes` section in the body — suspected area/module, suggested fix shape (not the code). Emit `bug-triaged` with `to: senior-developer-*` and `ref` to the bug file. One bug at a time, in M's order. **Never hand-edit the status marker — helper-only authorship per `_agent-protocol.md`.**
...
- `wake` (from `idle-monitor-*`, to: your exact ID) → idle-monitor detected your role's latest activity row is terminal and older than `PER_ROLE_IDLE_SECONDS`. Re-scan bus for missed events; run the wake-loop self-check above; then resume work, or — if genuinely idle (no story/bug/review pending) — call `mcp i_am_truly_idle({role, pid})` to give M ground truth (it gates `declare_idle`); your bit auto-invalidates on next activity. (May also be called proactively when you finish all in-flight work.) Closes 099's truly-idle limitation.
...
7. **Timing-ceiling lens (mechanism, not diligence).** At plan-review flag tight `wait_for`/poll ceilings; prefer generous ones (a tight ceiling times out under load). The real gates are `tests/lint-timing-ceilings.sh` (floors the `wait_for` ceiling at `WOW_MIN_WAIT_CEILING_S`) + `run-all.sh --repeat-timing` (the N×-flake gate) — not PP diligence.
