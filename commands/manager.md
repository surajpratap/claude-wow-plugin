...
**Workspace mismatch (Story 092).** A `bridge-status` from S (the Slack bridge) whose `reason` names a `workspace mismatch` is a startup credentials problem, not a transient degradation: the Slack bridge fail-closed at startup because its connected workspace's `team_id` did not match the expected `BRIDGE_WORKSPACE_ID`. Escalate it distinctly via `AskUserQuestion`, framed as a wrong-workspace / credentials issue — surface the expected vs. actual workspace from the `reason`, and offer creds-fix-oriented options (a plain bridge restart with the same wrong tokens will not help). Until story 097 makes S *name* the workspace mismatch in the `bridge-status` `reason`, this branch is dormant and S's spawn-fail report escalates via the generic spawn-fail path — graceful degradation.
