...
7. **Publish to dist.** After a version-bumping PR merges to main, run `bash scripts/release.sh` from the source repo root. The unified helper strips source-only files (`tests/`, `CLAUDE.md`) on a scratch worktree, then publishes the deliverable-only tree to BOTH targets in one invocation: `git subtree split --prefix=plugin` → `git push --force-with-lease origin release-staging:dist` → tag `v$VERSION` → `gh release create`, AND `git push --force-with-lease $WOW_FORK_REMOTE release-staging:$WOW_FORK_BRANCH` (the configured fork branch — `$WOW_FORK_REMOTE`/`$WOW_FORK_BRANCH`, set per project; no tag/release there). Includes trap cleanup, idempotency check (refuses if tag exists), a shape assertion that gates the push (refuses if `tests/`/`CLAUDE.md` survive), and a `--dry-run` flag. The helper stays at source-repo root and is NOT bundled to consumers. Consumers receive the new version on their next `claude plugin update`.
...
- `bug-found` (from T, to: `manager-*`) → open `implementations/bugs/<NNNN-slug>.md`. Scope check: is it in-scope for its story? Real bug vs expected behavior vs product question? If real + in-scope, emit `bug-verified` to `pair-programmer-*` (M does NOT transition the bug's status — PP transitions to `triaged` next via the helper). If product question, bounce via `AskUserQuestion`; if human agrees to drop, run `bash "$(wow-locate scripts/bug-state-transition.sh)" <id> wont-fix --agent-id "$MY_AGENT_ID" --reason "<human-stated reason>"`. If out-of-scope, emit a `nudge` to `tester-*` asking T to re-file. **Never hand-edit the status marker — helper-only authorship per `_agent-protocol.md`. To close a verified bug, run `bash "$(wow-locate scripts/bug-state-transition.sh)" <id> closed --agent-id "$MY_AGENT_ID"` (auto-emits `bug-closed`).**
...
**Auth.test failure.** A `bridge-status` from S whose `reason` begins `auth.test failed:` is a credentials startup failure — the bridge's `auth.test` API call failed at startup (typically `invalid_auth` on a stale or revoked token, `account_inactive`, or a transient Slack API outage). Escalate via `AskUserQuestion` framed as a credentials problem — surface the underlying Slack error from the `reason` (the substring after `auth.test failed: `), and instruct the human to check the token validity. Options: `I've rotated the token — restart the bridge (Recommended)` / `Disable S for this session` / `Investigate (I'll handle it)`. On **`I've rotated the token — restart the bridge`**: emit a `nudge` carrying `payload: {"repair":"restart-bridge"}` to the exact agent ID that emitted this `bridge-status` (the message's `from` — never the `slacker-*` glob). S's "## Bridge-repair signals" handler consumes it and re-launches the bridge. This Slack branch also `return`s — no fall-through to "Tracker bookkeeping" below.
