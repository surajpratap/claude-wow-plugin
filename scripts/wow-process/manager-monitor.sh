#!/usr/bin/env bash
# manager-monitor.sh — wrap manager-monitor.py with PID uniqueness.
#
# Started by M at session start as a wow-process. Single-instance per
...
SELF_LOCK="${PID_DIR}/manager-monitor.pid"
...
exec python3 "$SCRIPT_DIR/manager-monitor.py"
